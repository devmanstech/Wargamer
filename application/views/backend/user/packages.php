					...
					<div class="col-sm-4">
						<article class="album">
							...
							<div class="category-actions text-center">
								...
								?>
											<button class="btn btn-orange mt-4 mb-2 btn-rounded" onclick="showPaymentDiv('<?php echo $package['id']; ?>')"><?php echo get_phrase('choose_plan'); ?></button>
								<?php
										}
									}
								?>
							...
							<div class="row pl-1 pr-1 payment-section" style="display: none;" id = "payment-section-<?php echo $package['id'] ?>">
								<div class="col-12 text-center">
									<?php if($stripe[0]->active == 1){ ?>
										<a href = "<?php echo site_url('user/stripe_checkout/'.$package['id']); ?>" class="btn btn-primary btn-rounded" style="margin-bottom: 20px;" target="_blank"> <?php echo get_phrase('pay_with_stripe'); ?></a>
									<?php
									}

									if($paypal[0]->active == 1){
									?>
										<a href="<?php echo site_url('user/paypal_checkout/'.$package['id']); ?>" class="btn btn-primary btn-rounded" style="margin-bottom: 20px;" target="_blank"><?php echo get_phrase('pay_with_paypal'); ?></a>
									<?php } ?>
								</div>
							</div>
						</article>
